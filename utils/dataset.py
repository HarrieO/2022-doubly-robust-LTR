...
class DataFoldSplit(object):
  def __init__(self, datafold, name, doclist_ranges, feature_matrix, label_vector):
    ...
    self.query_index_per_document_vector = None

  def query_index_per_document(self):
    if self.query_index_per_document_vector is None:
      self.query_index_per_document_vector = np.zeros(self.num_docs(), dtype=np.int32)
      self.query_index_per_document_vector[self.doclist_ranges[1:-1]] = 1
      self.query_index_per_document_vector[:] = np.cumsum(self.query_index_per_document_vector) 
    return self.query_index_per_document_vector
