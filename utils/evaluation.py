...
def evaluate_policy(model,
  ...
  policy_scores = model(data_split.feature_matrix)[:,0].numpy()*model_output_multiplier
...
def evaluate_deterministic_ranking(
  ...
  doc_norm_factor = norm_factors[data_split.query_index_per_document(), :]
  doc_metric_values = (stacked_alphas[inverted_ranking, :]*doc_weights[:, None]
                       + stacked_betas[inverted_ranking, :]
                       )/doc_norm_factor

  metrics = np.mean(doc_metric_values, axis=0)*(doc_metric_values.shape[0]/float(data_split.num_queries()))
